...
bool timetable_set_slot(TimeTable *tt, WeekDay day, uint8_t period, 
    ...
    /* Guarantee null-termination across buffer limits */
...
/* Helper: parse a single CSV line into tokens via state machine */
static int parse_csv_line(char *line, char *fields[], int max_fields) {
    ...
    /* Strip surrounding quotes from captured fields */
    for (int i = 0; i < field_count; i++) {
        size_t flen = strlen(fields[i]);
        if (flen >= 2 && fields[i][0] == '"' && fields[i][flen - 1] == '"') {
            fields[i][flen - 1] = '\0';
            fields[i]++;
        }
    }
